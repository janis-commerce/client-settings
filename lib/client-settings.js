...
module.exports = class ClientSettings {
	...
	static async loadDefinition() {
		...
		if(!cache[`${this.session.clientCode}-${this.settingsPath}`])
			cache[`${this.session.clientCode}-${this.settingsPath}`] = new NodeCache({ stdTTL: this.cacheTime });
	}
	...
	static getFromCache(entity, settingName) {
		...
		if(!cache[`${this.session.clientCode}-${this.settingsPath}`].has(entity))
			return;

		return cache[`${this.session.clientCode}-${this.settingsPath}`].get(entity)[settingName];
	}
	...
	static async getForClient(entity, settingName) {
		...
		cache[`${this.session.clientCode}-${this.settingsPath}`].set(entity, settingsFormatted);
