...
module.exports = class PutSettingApi extends API {
	...
	async process() {
		...
		await this.updateClientSettings(dataToSave);

		const newSettings = addDefaults(this.defaultSettings, dataToSave);

		const topicArn = this.getTopicArn(newSettings);
		if(topicArn)
			await this.publishEvent(topicArn, newSettings);
	}
	...
	/**
	 * @param {Object} newSettings The new settings, including the default values
	 * @returns {string|null}
	 */
	getTopicArn() {
		return null;
	...
	/**
	 * @param {Object} newSettings The new settings, including the default values
	 * @returns {{content: Object, attributes?: Object}} The SNS Event (content and optional attributes)
	 */
	 ...
	 * @param {Object} newSettings The new settings, including the default values
	 */
	async publishEvent(topicArn, newSettings) {
		const snsTrigger = this.session.getSessionInstance(SnsTrigger);
		await snsTrigger.publishEvent(topicArn, this.formatEventData(newSettings));
	}
